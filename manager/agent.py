...
root_agent = Agent(
    ...
    description="A manager agent that delegates tasks to sub-agents for RAG (Retrieval-Augmented Generation) and SOP (Standard Operating Procedure) queries.",
    sub_agents=[
        rag_agent,
        sop_agent,
    ],
    instruction="""
    You are the manager agent. Your primary role is to understand the user's request and delegate it to the appropriate sub-agent. You have two sub-agents available:
    ...
    If the query is ambiguous, ask for clarification before delegating. If the query cannot be handled by either sub-agent, inform the user of your limitations.
    """,
)